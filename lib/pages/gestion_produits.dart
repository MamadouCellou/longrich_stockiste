...
import 'gestion_categories.dart';
...
class _GestionProduitsState extends State<GestionProduits> {
  ...
  Future<String> duplicateProduct(String productId) async {
    final result = await supabase.rpc(
      'duplicate_product',
      params: {'original_id': productId},
    );

    if (result == null) {
      throw Exception("Erreur lors de la duplication");
    }
                        ...
                        trailing: PopupMenuButton<String>(
                          onSelected: (val) async {
                            if (val == 'edit') {
                              ...
                              final newId = await _productService.duplicateProduct(p.id!);

                              print("Produit dupliqué ID : $newId");

                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text("Produit dupliqué ID : $newId"),
                                  backgroundColor: Colors.green,
                                ),
                              );
                            }
